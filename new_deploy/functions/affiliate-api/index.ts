// ============================================================
// affiliate-api — autossuficiente (sem imports de _shared)
  ...
  try {
    ...
    if (action === 'me') {
      const { data: sales } = await supabase.from('affiliate_sales')
        .select('id').eq('affiliate_id', aff.id).eq('status', 'paid')
        .not('asaas_subscription_id', 'is', null);
      const { count: totalClicks } = await supabase.from('affiliate_clicks')
        .select('*', { count: 'exact', head: true }).eq('affiliate_id', aff.id);
      const stats = {
    ...
    if (action === 'conversion-stats') {
      const { data: clicks } = await supabase.from('affiliate_clicks')
        .select('created_at').eq('affiliate_id', aff.id).order('created_at', { ascending: false });
      const { data: sales } = await supabase.from('affiliate_sales')
        .select('created_at, status').eq('affiliate_id', aff.id).order('created_at', { ascending: false });
    ...
    if (action === 'sales') {
      const { data } = await supabase.from('affiliate_sales').select('*')
        .eq('affiliate_id', aff.id).order('created_at', { ascending: false });
      return new Response(JSON.stringify({ data }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    if (action === 'commissions') {
      const { data } = await supabase.from('affiliate_commissions').select('*')
        .eq('affiliate_id', aff.id).order('created_at', { ascending: false });
      return new Response(JSON.stringify({ data }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (action === 'cancel-sale' && req.method === 'POST') {
      ...
      const { data: sale, error: saleErr } = await supabase
        ...
        .eq('id', saleId).eq('affiliate_id', aff.id).maybeSingle();
      ...
      const { error: updateErr } = await supabase.from('affiliate_sales')
      ...
      await supabase.from('affiliate_commissions')
        .update({ status: 'cancelled' }).eq('sale_id', saleId).eq('status', 'pending');

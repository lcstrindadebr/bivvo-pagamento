// ============================================================
// create-subscription — autossuficiente (sem imports de _shared)
// Cria assinatura PIX/Boleto no Asaas com cálculo Bivvo embutido.
// ============================================================
...
serve(async (req) => {
  ...
  try {
    ...
    if (bivvoConfig) {
      ...
      amount = q.total1m; recurringAmount = q.totalRec; planLabel = q.planLabel;
    } else {
    ...
    const sRes = await asaasFetch(`${ASAAS_BASE_URL}/subscriptions`, {
      ...
      headers: { 'Content-Type': 'application/json', 'access_token': ASAAS_API_KEY },
      body: JSON.stringify({
        customer: asaasCustomerId, billingType,
    ...
    if (billingType === 'PIX') {
      const pix = await asaasFetch(`${ASAAS_BASE_URL}/payments/${firstPayment.id}/pixQrCode`, { headers: { 'access_token': ASAAS_API_KEY } });
      paymentDetails = { pixQrCode: pix.encodedImage, pixCopyPaste: pix.payload, expiresAt: pix.expirationDate };
    } else if (billingType === 'BOLETO') {
      const bar = await asaasFetch(`${ASAAS_BASE_URL}/payments/${firstPayment.id}/identificationField`, { headers: { 'access_token': ASAAS_API_KEY } });
    ...
    const { data: dbPayment } = await supabase.from('payments').insert({
      user_id: user.id, plan, amount, status: 'pending',
    ...
    if (affiliateSlug && dbPayment) {
      ...
      if (aff) {
        const { data: sale } = await supabase.from('affiliate_sales').insert({
          ...
          commission_percent: aff.commission_percent, status: 'pending',
